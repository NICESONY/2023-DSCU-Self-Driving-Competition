...
#  ar 코드 개선, 속도 개선, 라인 주행 , 객체인식, 정지


#  (distance < 0.5 -> 0.8)  여기 값 변경: 이유는 미리 멈추게 하고 싶어서
# 계속 머리를 박더라고 
...
#==============================================
    ...
    # if (distance > 1.0):   # 1m 보다 멀면
    #     xx_pos = x_pos + 0.2  ###
    #     drive_angle = int(xx_pos*40) ### defelt = 100  전부 0.0 임
    ...
    # found = True
    # return found, drive_angle  


#  여기 값 전부 track_drive- 복사본 내용에서 참고해서 변경함

#=================================================

# def move_car(move_angle):
#     for i in range(200): 
#         drive(angle=move_angle, speed=Fix_Speed)
#         time.sleep(0.01)

# 여기를 2초 동안 이동하게 되어있는데 만약에 멀리 나가면 1초로 줄이든가 하자
# 또는 해당 함수의 내부로 들어가서 각도를 조금만 틀게 해보자 
# 여기 부분이 주차하는 부분이다.
...
# 라인에서 그 조정값으로 구하는 것 없애는 법 여쭤보자
    
# 여기 변경함 이렇게 해서 라인먼저 되는지 확인해 보자
...
#     if abs(new_angle) < 15:
#         new_speed = Fix_Speed + 2
#     elif abs(new_angle) > 35 :  ##  이거는 나중에 해보자
#         new_speed = Fix_Speed - 2


#     drive(new_angle, new_speed)

# 여기도 원래대로 3으로 변경하고 각도에 따라서 속도 조절하는 값을 넣음
...
#     drive(new_angle,new_speed - 7) #### 
#     retry_count = 0


# 장애물에서는 속도 있게 가고 ar은 안정적이게 
# 이후는 코너는 느리게 
...
# 1. 사진 찍고 충전하기
# 2. 충전하면서 라벨링
# 3. 내가 찍은 것과 받은 파일 70퍼 30퍼 로 만들고 10000으로 학습 시작
# 4. 학습은 원격으로 돌리고 나는 자동차 자율 주행 시작 
# -  먼저 속도 올린 부분 잘 지나가게 만들고 
# -  코너 천천히 만들고
# -  ar에서 느리게 만들어서 인식 시키면서 이동시키고
# -  이후는 라인 
# -  객체 인식
# -  주차 까지 완료 시키기
...
# 충남대 경주대회용으로만 사용 가능하며 외부 유출은 금지됩니다.
...
#=============================================
CAM_FPS = 23  # 카메라 FPS 초당 30장의 사진을 보냄
WIDTH, HEIGHT = 640, 480  # 카메라 이미지 가로x세로 크기
...
#=============================================
avg_count = 5  # 이동평균값을 계산할 데이터 묶음 갯수    ##############여기도 바꾸는 것########3
...
def ultra_callback(data):
    ...
    # get_mmed() 여기를 위에서 사용하고 싶은거 고르는 것################3
    #################get_wmavg#################
#=============================================
...
def check_traffic_sign():
    ...
    cimg = image.copy()
    Center_X, Center_Y = 300, 100  # ROI 영역의 중심위치 좌표 
    XX, YY = 190, 100  # 위 중심위치 좌표에서 좌우로 XX만큼씩, 상하로 YY만큼씩 벌려서 ROI 영역을 잘라냄   
...
def start():
    ...
    while not rospy.is_shutdown():
        ...
        while drive_mode == LANE_DRIVE:
            ...
            if (distance < 0.5) and (ar_ID == 1): # AR_Drive Sign
            ...
            elif (distance < 0.8) and (ar_ID == 2): # Obejct Detection Sign
